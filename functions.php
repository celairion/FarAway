...
***************************************************************************/
	...
	DEFINE("PROJECTTHEME_VERSION", "4.5.5.2");
	DEFINE("PROJECTTHEME_RELEASE", "20 Dec 2022");
...
//---------
	...
	include 'lib/widgets/front-page-main-slider-function.php';
...
*/
	...
	function ProjectTheme_approved_methods( $citymethods ) {
	    $citymethods['text'] = 'Active User';
	    return $citymethods;
	}
	add_filter( 'user_contactmethods', 'ProjectTheme_approved_methods', 10, 1 );


	function new_custom_user_table( $column_city ) {
	    $column_city['text'] = 'Active User';
	    return $column_city;
	}
	...
	function ProjectTheme_new_custom_user_table_row( $value, $column_city, $user_id ) {
	    switch ($column_city) {
	        case 'text' :
	            $x = get_the_author_meta( 'active_user', $user_id );
							if($x == "no") return "<span style='color: red'><b>No</b></span>";
							else return "<span style='color: green'><b>Yes</b></span>";
	        default:
	    }
	    return $value;
	}
	...
	add_filter( 'manage_users_custom_column', 'ProjectTheme_new_custom_user_table_row', 10, 3 );
...
add_action('init', 'pt_wps_change_role_name');
	...
	$width_widget_categories = 300;
	$height_widget_categories = 370;
...
//------------------ file includes -----------------------------
	...
	get_template_part('lib/admin_menu');
 	get_template_part("lib/my-account/favourites");
...
if(!function_exists('project_theme_get_name_of_user'))
{
...
if(!function_exists('ProjectTheme_get_user_table_row_admin'))
{
...
if(!function_exists('ProjectTheme_get_user_table_row_widget'))
{
...
*
**************************************************************/
if(!function_exists('ProjectTheme_get_user_table_row'))
{
...
function ProjectTheme_add_theme_scripts()
{
  ...
  $new_Project_step =  get_query_var('post_new_step');
  $p_action			= get_query_var('p_action');
...
if(!function_exists('pt_get_user_role_nice_string'))
{
...
function ProjectTheme_my_custom_columns($column)
{
	...
	elseif ("price" == $column)
	{
		$hourly_paid = get_post_meta($post->ID,'hourly_paid',true);
		if($hourly_paid == 1)
		{
...
<?php
					...
					if($type == "input_box")
					{
							...
							<input type="number"   size="50" class="form-control" name="budgets" placeholder="<?php echo ProjectTheme_get_currency() ?>" value="<?php echo $sel; ?>" />
       ...
       <?php endif; ?>
			 ...
			 $ProjectTheme_enable_hourly_rate = get_option('ProjectTheme_enable_hourly_rate');
			 if($ProjectTheme_enable_hourly_rate == "yes")
			 {


				 $hourly_paid = get_post_meta($pid,'hourly_paid',true);
			...
			<h2><input type="checkbox" name="hourly_paid" value="1" <?php echo $hourly_paid == 1 ? "checked='checked'" : "" ?> /> <?php echo __('Is this an hourly paid project ?', 'ProjectTheme'); ?></h2>

			<?php

				 $hourly_rate = get_post_meta($pid,'hourly_rate',true);

			?>
			<input type="number"   size="50" class="form-control" name="hourly_rate" placeholder="<?php echo ProjectTheme_get_currency() ?>" value="<?php echo $hourly_rate; ?>" />
...
function ProjectTheme_add_query_vars($public_query_vars)
{
		...
		$public_query_vars[] = 'project_author';
...
add_image_size( 'avatara_profile', 190	, 190, true ); //category images in the widget
...
function ProjectTheme_get_avatar_new_2021($uid)
{
	$av = get_user_meta($uid, 'avatar_' . 'project', true);
	if(empty($av)) return get_template_directory_uri()."/images/noav.jpg";
	else return ProjectTheme_generate_thumb_av($av, 'avatara_profile');
}
...
*
**************************************************************/
function projectTheme_number_of_submissions_contensts($pid)
{
	global $wpdb;
	$s = "select bid from ".$wpdb->prefix."project_contest_submissions where pid='$pid'";
	$r = $wpdb->get_results($s);
...
if(!function_exists('ProjectTheme_project_get_star_rating'))
{
	function ProjectTheme_project_get_star_rating($uid)
	{
		...
		if(count($r) == 0)	return ProjectTheme_get_project_stars(0) . " (0)";
		else
		foreach($r as $row) // = mysql_fetch_object($r))
		{
			$i++;
...
function ProjectTheme_get_user_profile_link($uid, $pg = '')
{
	...
	else return home_url(). '/?p_action=user_profile&project_author='. $uid . "&profile_pg=" . $pg;
}
/*************************************************************
*
*	ProjectTheme (c) sitemile.com - function
...
*
**************************************************************/
function projectTheme_get_service_front_end ( $arr = '')
{
	$pid = get_the_ID();
	global $post;
					...
					<div class="row">
						...
						<div class="project-title-list col-sm-3 col-md-3">
									...
									<p class="mb-2 budget-font"><?php echo sprintf(__('<i class="fas fa-money-bill-alt"></i> %s','ProjectTheme'), $budget) ?> </p>
															<p class="mb-2"><a href="<?php the_permalink() ?>" class="btn btn-block btn-primary"><?php echo __('Buy Service','ProjectTheme'); ?></a></p>
...
function projectTheme_get_project_front_end ( $arr = '')
{
	...
	{
		$budget_num = get_post_meta($pid,'budgets',true);
		$budget = ProjectTheme_get_budget_name_string_fromID($budget_num);
	}
...
<div class="d-flex flex-row">
		<div class="flex-grow-1"><h2 class="my-title-prj"><a href="<?php the_permalink() ?>"><?php the_title() ?></a></h2></div>
		<div class="flex-shrink-1">
		
		<a href="<?php the_permalink() ?>" class="btn btn-sm btn-primary my-radius-added"><?php _e('View Trip Details','ProjectTheme') ?></a>
					...
					<div class="row">
						<div class="project-title-list col-sm-12 col-md-12">  

<div class="contest-row2">
						<ul class="my-details" style="padding:0; margin: 0">
				<li><?php _e('Budget:','ProjectTheme') ?> <span class="text-success font-weight-bold"><?php echo $budget ?></span></li>
				<li><?php _e('Likes:','ProjectTheme') ?> <span class="font-weight-bold badge badge-primary"><?php echo $proposals ?></span></li>
				<li><?php

								if($ending < 0)
								echo  sprintf(__('Expired','ProjectTheme'));
								else
								echo  sprintf(__('<i class="fa fa-clock"></i> %s left','ProjectTheme'), ProjectTheme_prepare_seconds_to_words($days_left))  ?> </li>
				 
				</ul></div>

						<p class="excerpt-post"><?php the_excerpt() ?></p>
							
							<a href="<?php the_permalink(); ?>"><img width="125" height="125" style="float: right" class="image_class"
...
function projectTheme_number_of_likes($pid)
{
  global $wpdb;
  $s = "select count(id) total from ".$wpdb->prefix."classified_watchlist where pid='$pid'";
  $r = $wpdb->get_results($s);
...
//--- begin subscription functions for paypal -------------

function PT_CallAPI($method, $url, $data = false)
{
    $curl = curl_init();
    ...
    curl_setopt($curl, CURLOPT_USERPWD, get_option('project_theme_paypal_client_id') . ":" . get_option('project_theme_paypal_client_secret') );
...
function PT_paypal_get_access_token()
{
		$res = PT_CallAPI('POST', "https://api-m.sandbox.paypal.com/v1/oauth2/token", "grant_type=client_credentials");
		$res = json_decode($res);
...
function PT_create_product_paypal_subscription()
{
		$url = "https://api-m.sandbox.paypal.com/v1/catalogs/products";
		$access_token = PT_paypal_get_access_token();

		$data = '{
					  "name": "Membership Subscription",
					  "description": "Membership Subscription Service",
					  "type": "SERVICE",
					  "category": "SOFTWARE",
...
function PT_create_plan($cost)
{
	$access_token = PT_paypal_get_access_token();
	$url = "https://api-m.sandbox.paypal.com/v1/billing/plans";
	$product = PT_create_product_paypal_subscription();
...
function PT_paypal_create_subscription($cost)
{
	$access_token = PT_paypal_get_access_token();
	$url = "https://api-m.sandbox.paypal.com/v1/billing/subscriptions";
	$planID = PT_create_plan($cost);
...
if(isset($_GET['test_curl_paypal']))
{

	$subscription = PT_paypal_create_subscription(10);
...
if(isset($_GET['pay_contact_details_fee_paypal']))
{
		...
		$paagee 	=  get_query_var('my_custom_page_type');
		$p_action 	=  get_query_var('p_action');
...
function projectTheme_rewrite_rules( $wp_rewrite )
{
		...
		'user-profile/([^/]+)/?$' => 'index.php?p_action=user_profile&project_author='.$wp_rewrite->preg_index(1),
		'user-feedback/([^/]+)/?$' => 'index.php?p_action=user_feedback&project_author='.$wp_rewrite->preg_index(1)
