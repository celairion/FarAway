		...
		<!-- page content here -->
				...
				if(count($r) > 0)
				{
					...
					foreach($r as $row)
					{
						...
						$unread = projectTheme_get_total_message_thread($row->id);

						?>


						<li>
					<a href="<?php echo ProjectTheme_get_priv_mess_page_url('read-message', $row->id) ?>">
						<div class="message-avatar">
							<img src="<?php $av = ProjectTheme_get_avatar($user_nv->ID,	35, 35); echo $av; ?>" alt="">
						</div>
						<div class="message-body">
							<div class="message-body-heading">
								<h5><?php echo $user_nv->user_login ?><?php if($unread > 0) { ?><span class="unread"><?php _e('Unread','ProjectTheme') ?></span><?php } ?></h5>
								<span><?php echo date_i18n('d-M-Y H:i:s',$row->datemade) ?></span>
							</div>
							<p><?php echo substr($last_mess,0,45) ?></p>
						</div>
					</a>
				</li>
