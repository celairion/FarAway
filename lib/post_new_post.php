...
if(isset($_POST['project_submit1'] ))
{
		...
		$hourly_paid = sanitize_text_field($_POST['hourly_paid']);
		$hourly_rate = sanitize_text_field($_POST['hourly_rate']);

		update_post_meta($pid, 'finalised_posted', '0');
		update_post_meta($pid, 'hourly_paid', $hourly_paid);
		update_post_meta($pid, 'hourly_rate', $hourly_rate);
		...
		if($hourly_paid == 1 and empty($hourly_rate))
		{
			$projectOK 							= 0;
			$MYerror['hourly_paid'] 					= __('Please type in your desired hourly rate.','ProjectTheme');
			$class_errors['hourly_paid']		= 'error_class_post_new';
		}
